---
title: "Auth API: Register, Login, and Account Management"
sidebarTitle: "Auth"
description: "REST endpoints for user registration, login, token refresh, account info, plan changes, password updates, and account deletion."
---
...
Exchanges a valid refresh token for a **new access token**. The response contains only `accessToken` — no `refreshToken` and no `user` object. No rate limit is applied to this endpoint.
