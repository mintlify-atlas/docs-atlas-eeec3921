---
title: "Authenticate with the Quikko API Using JWT"
sidebarTitle: "Authentication"
description: "Quikko uses JWT Bearer tokens for API authentication. Obtain an access token via register or login, then pass it in the Authorization header on every request."
---
...
To sign in with an existing account, call `POST /api/v1/auth/login` with the same body shape. The response is identical in structure. Both `register` and `login` are rate-limited per IP to protect against brute-force attacks, with independent counters configurable via `AUTH_RATE_LIMIT_PER_MIN` and `LOGIN_RATE_LIMIT_PER_MIN`.
