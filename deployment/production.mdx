---
title: "Deploy Quikko to Production"
...
Pass all required environment variables with `-e` flags or via an `--env-file`. At minimum you must supply `MONGO_URI`, `INFLUX_TOKEN`, and `JWT_SECRET` — the server will exit immediately without them.
