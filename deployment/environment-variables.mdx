---
title: "Environment Variables Reference for Quikko"
...
<Warning>
  `MONGO_URI`, `INFLUX_TOKEN`, and `JWT_SECRET` are required. The server exits
  at startup if any of these three variables are missing or empty.
</Warning>
...
<ParamField path="INFLUX_TOKEN" type="string" required>
  InfluxDB API token. Required — must match the token configured when the
  InfluxDB instance was initialised. In the Docker Compose setup this is
...
<ParamField path="JWT_ACCESS_TOKEN_TTL" type="duration" default="15m">
  Lifetime of short-lived access tokens. Accepts Go duration strings suffixed
  with `s`, `m`, or `h` (e.g. `15m`, `1h`).
</ParamField>
...
<ParamField path="JWT_REFRESH_TOKEN_TTL" type="duration" default="7d">
  ...
  `JWT_ACCESS_TOKEN_TTL`, plus a `d` suffix for days (e.g. `7d`, `30d`).
</ParamField>
